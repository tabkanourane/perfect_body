...
import 'package:perfect_body/plugins/notification_plugin.dart';
import 'package:perfect_body/services/firestore/firestore_notification_service.dart';
...
class NotificationBloc implements BlocBase {
  ...
  Function(List<NotificationData>) get _inNotifications =>
      _notificationsController.sink.add;
  Stream<List<NotificationData>> get outNotifications =>
      _notificationsController.stream;

  Future<void> init() async {
    final notificationStream =
        await FirestoreNotificationService.getAllNotifications();
    notificationStream.listen((querySnapshot) {
      _notifications = querySnapshot.documents
          .map((doc) => NotificationData.fromDb(doc.data, doc.documentID))
          .toList();
